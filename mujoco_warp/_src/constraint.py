...
###############################################################################
# Three-kernel contact constraint pipeline (pyramidal)
###############################################################################


@wp.kernel
def _efc_contact_pyramidal_init(
  # Model:
  ...
  """Init kernel: activate contacts, allocate EFC rows, compute sparse structure."""
  conid, dimid = wp.tid()
  ...
  # Sparse row structure: count non-zeros
  if is_sparse:
    geom = geom_in[conid]
...
def _efc_contact_jac_tiled(tile_size: int, cone_type: types.ConeType):
  """Tiled contact Jacobian kernel from PR #1087.

  Processes DOF blocks in tiles across (worldid, dof_block_id) dimensions.
  For each DOF block, iterates over all contact EFC rows and computes the
  Jacobian using vectorized tile operations with dof_affects_body mask.
  """
  TILE_SIZE = tile_size
...
@wp.kernel
def _efc_contact_pyramidal_update(
  # Model:
  opt_timestep: wp.array(dtype=float),
  opt_disableflags: int,
  opt_impratio_invsqrt: wp.array(dtype=float),
  ...
  """Update kernel: set EFC row values (D, aref, vel, etc.) using precomputed Jqvel."""
  conid, dimid = wp.tid()
...
###############################################################################
# Original monolithic contact kernels (kept for elliptic path)
###############################################################################
...
def _contact_pyramidal(
  ...
  if active:
    ...
    # precompute offsets once (instead of per-DOF inside jac_dof)
    offset1 = con_pos - wp.vec3(subtree_com_in[worldid, body_rootid[body1]])
    offset2 = con_pos - wp.vec3(subtree_com_in[worldid, body_rootid[body2]])
    ...
    while True:
      ...
      if dofid == da:
        # inlined Jacobian: read cdof once, compute jacp/jacr difference directly
        cdof = cdof_in[worldid, dofid]
        cdof_ang = wp.spatial_top(cdof)
        cdof_lin = wp.spatial_bottom(cdof)

        affect1 = dof_affects_body[body1, dofid]
        affect2 = dof_affects_body[body2, dofid]

        jacp_dif = wp.vec3(0.0)
        jacr_dif = wp.vec3(0.0)
        if affect2 != 0:
          jacp_dif = cdof_lin + wp.cross(cdof_ang, offset2)
          jacr_dif = cdof_ang
        if affect1 != 0:
          jacp_dif = jacp_dif - cdof_lin - wp.cross(cdof_ang, offset1)
          jacr_dif = jacr_dif - cdof_ang
        ...
        for xyz in range(3):
          ...
          if condim > 1:
            if dimid2 < 3:
              ...
              Ji += frame[dimid2 - 3, xyz] * jacr_dif[xyz]
  ...
  if not (m.opt.disableflags & types.DisableBit.CONSTRAINT):
    ...
    if not (m.opt.disableflags & types.DisableBit.CONTACT):
      if m.opt.cone == types.ConeType.PYRAMIDAL:
        # 3-kernel contact pipeline: init → jac → update
        wp.launch(
          _efc_contact_pyramidal_init,
        ...
        # Tiled Jacobian kernel: processes DOF blocks in tiles
        tile_size = m.block_dim.contact_jac_tiled
        n_dof_blocks = (m.nv_pad + tile_size - 1) // tile_size

        # Reinterpret frame (mat33 → (n, 3) of vec3) and friction (vec5 → (n, 5) of float)
